# Filecoin Pay — Fee &amp; Burn Model

> An interactive model and supporting analysis of fee-driven FIL burn in the **Filecoin Onchain Cloud (FOC)**.
...
This project documents how Filecoin Pay fees translate into FIL burn, quantifies each category from on-chain data, and provides an interactive calculator that projects fee revenue and FIL burn under network-growth scenarios.

The calculator is a single, self-contained HTML page. It reconstructs the FPV (Filecoin Pay Volume) growth models from the FIP-1249 scenario explorer and overlays the fee structure for three configurable regimes — **mainnet v1.2.x**, **v1.3.0**, and a tunable third regime — so the burn contribution of each lever can be compared side by side.

## Key findings

| Finding | Detail |
|---|---|
| The 0.5% network fee scales with volume | At today's volume it is marginal (≈ **2 USDFC** across all of mainnet history); under the growth scenarios it becomes the dominant percentage-based source. |
| On mainnet (v1.2.x) the createDataSet "sybil" fee funds the pool | ≈ **74.8 USDFC** (≈ **97%** of the auction pool), converted to **52.7 FIL** across 32 Dutch auctions. |
| Under v1.3.0 the createDataSet fee is paid to the SP, not burned | The 0.1 USDFC burn rail stops at the calibnet v1.3.0 upgrade (last burn-sink payment 2026-06-08); the fee becomes 0.025 USDFC accrued to the provider. Fee-driven burn then comes from the network fee and the proof fee. |
...
| Other tokens convert at a lower efficiency | Fees in tokens other than FIL/USDFC convert to a FIL burn through the same auction, but at a Dutch-auction discount and delay relative to USDFC. |

Full data and the quantitative calibration analysis are in [`Filecoin-Pay-Fee-Redesign.md`](./Filecoin-Pay-Fee-Redesign.md).
...
| [`Filecoin-Pay-Fee-Redesign.md`](./Filecoin-Pay-Fee-Redesign.md) | The analysis: fee/burn taxonomy, on-chain figures, and fee-calibration math. |
| `README.md` | This file. |
...
2. **Fee regime** — switch between v1.2.x / v1.3.0 / configurable presets and tune the network fee, createDataSet fee and its destination (burned vs. paid to SP), and the proof-fee rate.
3. **Token mix &amp; non-FIL surcharge** — set the share of volume settled in FIL / USDFC / other tokens, and the surcharge, conversion efficiency, and auction efficiency applied to them.
...
git commit -m "Filecoin Pay fee & burn model"
git branch -M main
...
- **Defaults** are anchored to observed on-chain data (Nov 2025 – Jun 2026); e.g., the proof-fee branch reproduces the measured ≈ 0.52 FIL/quarter at the current ≈ 25 TiB actively proven.
- **USDFC → FIL conversion** approximates the fee auction as `FIL burned ≈ pool ÷ FIL price`, with adjustable conversion efficiency per token.

## Assumptions

- **Fee auctions close.** Every `burnForFees` auction pool is eventually cleared, so accrued fees in any token do convert to a FIL burn. Tokens differ in the *efficiency and timeliness* of that conversion (FIL burns immediately at par; USDFC clears near par on the ~weekly auction; other tokens clear at a discount and/or less frequently) — not in *whether* conversion happens.
- Storage-rate, proving-frequency, and dataset-size inputs are exposed as tunable parameters rather than hard-coded.
